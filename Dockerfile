...
ARG USER_NAME="eric-oss-performance-monitoring-enabler"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-performance-monitoring-enabler-app.jar
...
LABEL \
    org.opencontainers.image.title=eric-oss-performance-monitoring-enabler-jsb \
